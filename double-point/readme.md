...
* 左右指针
    * 二分
    * 两数之和（对于有序数组，根据目前左右指针对应元素之和与target的大小，进行调整是，左指针前进时（增大和），还是右指针后退（减小和）
    * 反转数组
    * 滑动窗口：一般是left right是左闭右开，右指针每向前滑动一次便形成一个候选解，滑动到某个程度构成一个可行解，在此基础上左指针开始滑动，以进行优化以得到最终解（有时候不是为了优化而是右指针的滑动导致解变的不可行）  
...
* 快慢指针
    * 判断链表中是否有环
    * 判断有环链表中环的起始位置：快慢指针相遇之后（相遇点所在位置距离头指针为环的长度l），一个从链表头出发，一个从相遇点出发，以相同的速度前进， 再次相遇的位置就是环的起始位置(假设环的起点距离头指针为m,则显然再次到环起点的时候，从相遇点出发的指针也走了m步，其现在的位置如果套圈则为l+m-l为m,因为在次相遇肯定是套圈了的，则两个指针会相遇在环的起点处
    * 寻找链表中点，这可用于链表的合并排序
    * 寻找倒数n个点的指针，可以先让快指针前进n步，然后两个指针同时前进
## 去重移动类（快慢指针）
### lc26
