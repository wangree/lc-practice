...
滑动窗口中单调队列的实现：由于一般滑动窗口只关心最大或者最小值，比如这里假设是最大值，那么在加入一个元素的时候，队列中在他之前的元素且比它小的元素都是不可能再成为最大值的了，所以这里可以直接不要这些值，把新元素放在队尾；而删除元素的时候，仅仅在该元素最大时才删除，因为在其他情况下它都不会影响最大元素的产生
