...
# P、NP和NP完全问题
## p问题
能够在多项式时间内求解的问题，都是判定问题，也就是p问题，就是说p问题一定是判定问题；但判定问题不一定都在多项式时间内可解，不一定是p问题；不可判定的那一类判定问题称作不可判定问题，如[停机问题](https://baike.baidu.com/item/%E5%81%9C%E6%9C%BA%E9%97%AE%E9%A2%98/4131067?fr=aladdin),在这里也稍作证明：
...
从另外一个角度讲，p是能够在多项式时间内解决的所有搜索问题的集合，所以可以说p描述了目前能够保证在合理的时间范围内解决的所有问题的集合。
## 不确定算法和NP类问题
也存在可以判定有没有解但难解的问题，这种情况及其少见；但更多的是找不到多项式类型的算法的问题，也无法证明这类算法不存在（无法判定，对应于决定性问题），书中列举有（都是np-hard问题）：
...
不确定算法求解的判定问题为NP类问题，NP的全称是nonderterminism problem,意思是当一个算法面对若干个选项时，它有能力猜出正确的选择，就是不确定性的生成一些解然后再去验证这些解是否成立；
