...
经过以上策略，算法一定会到达l1,l2并将其元素尽数比完，问题下界即O(2n−1)，总结一下就是将我们的输入中k在中间的两条斜对角线上时，这其实就是step-wise的
## 问题化简
...
### 信息论下界例子
一堆硬币中有一个较轻的硬币，用天平称重，并不是log2N为准确下界，因为从信息论角度，如果把硬币分成三堆，一次称重可以得到比两堆更多的信息
### 证明基于相邻元素排序的方法下界为n(n-1)/2
对于此类算法主要是要把一个倒置交换回来，最差情况是数组是倒序的，这种情况下i位置元素有i-1个倒置，总计有n(n-1)／2个倒置因而要交换这么多次
### 对手论证证明连通性下界
我们可以对一个有N个点的图做如下操作，将它分为两个子集，一个为n大小U1，另一个为m大小U2，算法如果在询问是否存在边u,w(u属于U1,w属于U2)之前算法检查出图是连通的，则我们将图变为U1，U2中全联通,U1和U2中所有有边都打断；或者算法这时候检查出图是不联通的，则将u,w
联通起来，U1和U2都联通起来，那么我们发现这两种情况，都至少要再进行n*m次询问边是否存在，而我们要让询问次数最多，即输入最差，则要使得n=m,则不难得到算法的下界了
# 问题的分类
