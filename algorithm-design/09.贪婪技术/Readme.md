...
### quick-find
Quickfind 每个节点都指向自己所属根节点，find非常快，但union的时候需要遍历所有节点;每次find只需要访问数组一次，而uinion操作会调用两次find并检查所有N个元素并改变需要改变的元素（每次改变最多需要两次访问两次，读一次，改一次），因而访问数组的次数在N+3和2N+1之间
### quick-find的改进
...
### Bellman-ford算法的证明
首先肯定存在一条S到t(任意顶点)的最短路径，上面的点V_0,...V_i,V_k V_k为t,首先如果这是一条最短路径，利用反证法可以知道这条也是上面其他所有点的最短路径；因而利用归纳法,I=0时显然该算法可以得到V_0最短路径；对于I,进行放松后，V_i+1的距离会更新为v_i+e(V_i,V_i+1),也就是V_i+1的最短距离，问题得证，关键就是每一轮至少能够放松一个点.  
该算法实际可行的版本，首先初始化起点距离为0，其他为无穷，将起点加入队列，出队点p,并设置其状态为不在队列中，然后如果目前没有环路，对其邻近点不在队列中且进行放松操作,放松成功(更新了距离和前继边)的加入队列，并记录其已经在队列中,队列为空为止；判断环路在放松操作中，每成功放松V个点，对于每个点的前继边构成的图进行环路查找，有环路则算法不进行放松报错.  
环路查找的实现是对每个点进行dfs遍历，如果遍历到了一个先前的点，则证明有环路。  
那为什么这里只进行环路的判断而不是负环路呢？这是因为对于该算法而言，放松前v个点时可能形成的环路是仅仅可能是负环路，正环路无法放松成功，所以只可能形成负环路;再加上V是负环路的可能的最大周长，所以在每放松v个点时进行负环路的查找
### 套汇与有向图
汇率之间可以形成有向图,货币A兑换成B,用a到b的边的权重代表该汇率。对其取负对数，则可以转换为最短路径问题，如果a到b到c的汇率之积最大代表最短路径，这时候可以获得更多的c,负权重环代表本币套汇的机会
### 如何在（加权）无向图中找到最短路径？
对于边的权重均为正的图，Dijkstra 算法可以解决这个问题。只需根据给定的 EdgeWeightedGraph 构造一幅 EdgeWeightedDigraph （无向图中的每条边都对应着有向图中的两条方向不同的边）并执行Dijkstra 算法即可。如果边的权重可能为负，高效的算法也是存在的，
但它们比 Bellman-Ford 算法更复杂。
# 哈夫曼树及其编码
...
然后在具体生成一个字母的编码时，左边记录为0右边记录为1,便可以生成一串二进制编码，按照这种树，每个字母生成编码遍历的深度(也就是生成编码的长度)应该是比较短的；而且因为所有字母都在叶节点所以不会出现一个字母是另一个字母的前缀的情况；压缩率是指根据现概率构造的树与相比概率均等情况下构成的树的平均深度之比
## 哈夫曼树的特性
概率最小的两个节点具有相同的深度；频率高的字符码长总是小于等于频率低的；哈夫曼树最高可能有n-1
## 线性效率的哈夫曼树构造算法
首先前提是有一个已经按频率排好序的字母表作为第一个数组
第二个数组存放新树的权重，一开始为空；每次迭代，在两个数组中找出最小的两个(只需要在两个数组中的前两个中找，因为新树根节点的频率总是非递减的),把他们的和放进第二个数组，并从原数组删除，每次取出的两个数字去构造新树；依次循环进行n-1次迭代
## 如何在不实际构造一棵哈夫曼树的情况下生成一套哈夫曼编码
我们可以通过以下直接源自霍夫曼算法的方法从右到左生成码字：当两棵树合并时，在左子树中每个叶子的当前位串前面附加 0 并在右子树中每个叶子的当前位串前面附加 1 。 （假设与初始单节点树关联的子串为空。） 
### 二叉树特性
对于有n个叶节点的完全二叉树（要注意其形状并不固定），总节点数为2n-1，哈夫曼树就是一颗完全二叉树
