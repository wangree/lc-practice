...
* 可以通过证明其逆否命题：M不是最大匹配，M存在增益路径，反证法M非最大匹配,假设不存在增益路径。如果M非最大匹配，设M<sup>+</sup>为最大匹配，则M与M<sup>+</sup>的对称差形成的图中的联通分量中含有M+部分的，要么是单独一条边，这显示是一个增益路径，要么是交替路径，由于M+数量更大，要么是存在单条M+中的边，要么存在交替路径，在只有交替路径的情况下总能找到起点变和终点边属于M+的交替路径，因而总能发现一条增益路径，假设不成立
## 二分图寻找最大匹配的算法
