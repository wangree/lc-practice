...
## 算法效率
该算法的效率是O(EV^2/2),一条增广路径至少有一个关键边，所以我们可以用关键边数量来估计增广路径的个数，由于每条边能够成为关键边至少会让这个边某点到s的距离增加2，也就是路径长度增加2,而一条路径到最后最多长度是V,所以一条关键边最多会成为V/2次关键边，所以经过它的增广路径最多有EV/2条，所以最终是EV^2/2,具体见https://www.cnblogs.com/uid001/p/10465580.html
# 二分图的最大匹配
## 一些匹配的概念
* 增益路径
* 不饱和点（自由顶点）
