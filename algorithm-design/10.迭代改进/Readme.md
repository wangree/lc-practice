...
要证明这个匹配是稳定的匹配，则可以假设现在还存在受阻对m,w，m比起现在的配偶一定更倾向于w，说明w一定拒绝过或者换掉过m,那么w的最终配偶的优先级要高于m,说明这并不是一组受阻对，假设不成立，说明这是一个稳定的婚姻匹配；稳定婚姻算法有一个明显的缺陷。它有性别倾向，算法中女性一开始是无条件接受的，直到有人向她求婚才有可能换掉当前配偶。而这个倾向是无法消除的，从侧面说明该算法生成的最优匹配是唯一的。
