...
### 求树的最大匹配
 取任意叶结点 v 并将其匹配到它的 parent.p。 从树中删除 v 和 p 以及所有与 p 相关的边。 如果有的话，还要删除 v 的所有前兄弟叶子。 对于获得的较小的森林，该操作将递归地重复。 
 具体伪代码如下：
 ```cpp
 //在自由树中构造一个最大匹配
//输入：一棵树T  
//输出：在T中匹配M的最大基数  
将匹配M初始化为空集  
从任意顶点开始，通过 BFS 遍历T，对访问过的点编号
顶点从 0 到 n - 1 顺序并保存指向已访问对象的指针
顶点及其父节点     
for v ← n − 1 down to 0 do  
    如果顶点编号为i及其父节点均未标记  
    将它们之间的边添加到M  
    标记父节点  
return M  
...
## 稳定婚姻问题（有优先级的二分图匹配）
一个婚姻匹配M是一个包含n个（m,w)对的集合，每一对的成员都按照一对一的模式从两个不相交的n元素集合Y和X中选出。如果对于m和w,他们都更加倾向于对方而不是目前的匹配对象则称之为受阻对，如果M中不存在受阻对则称其为稳定的，否则就是不稳定的，现在就是要求一个稳定的婚姻匹配。  
对于此类问题，对于每一个男士或者女士都有一个优先列表。  
该算法简单可以描述为：
自由男生按女生优先级对没有拒绝过他的女生求婚，自由直接接受，不自由则跟当前配偶比较，更好则换掉当前配偶，否则拒绝求婚，男生完成匹配后到下一个男生，如此循环，直到没有自由男生
### 效率分析及正确性证明
 显然是O（n^2)的算法，在最差的情况下，一方面每个男士不可能向每个女士求婚两次，故求婚次数上限为n^2；另一方面，如果每个人有相同的优先列表，则求婚次数总共为$ sum_{i=1}^n {i}  =  n(n+1)/2$,故最差情况下的求婚次数在两者之间；最优情况是每位男士的第一选择都不同，算法只需要运行n次。  
要证明这个匹配是稳定的匹配，则可以假设现在还存在受阻对m,w，m比起现在的配偶一定更倾向于w，说明w一定拒绝过或者换掉过m,那么w的最终配偶的优先级要高于m,说明这并不是一组受阻对，假设不成立，说明这是一个稳定的婚姻匹配；稳定婚姻算法有一个明显的缺陷。它有性别倾向，算法中女性一开始是无条件接受的，直到有人向她求婚才有可能换掉当前配偶。而这个倾向是无法消除的，从侧面说明该算法生成的最优匹配是唯一的，和男士求婚顺序无关。  
另外一点需要注意的是男士优先的匹配算法得到的匹配结果M是所有稳定匹配结果中女士的结果最差的，这里依然可以用反证法证明，假设还有一个$\bar M$中对于w的配偶m_'比在M中的配偶m更好，则由于M是男士优先得到的，则M中m的配偶w一定是比$\bar M$中的配偶w_'要好的，则这时候对于$\bar M$，(m,w)是一对受阻对，与假设矛盾。
### 室友问题
偶数个男孩要分成一对对室友，在配对的集合中，如果有两个男孩不是室友，但相对于现在室友他们都要更希望彼此，那么该集合是不稳定的，反之该集合是稳定的。这称为室友问题。该问题不像婚姻匹配问题一定有解答，有可能无解。比如有个男孩x是大家都最不喜欢的，则一定会有一个人比如是p和他配对，而p又是其他人q的最佳选择，则q,p构成了一个受阻对使得问题不稳定
