...
输出以上算法的结果的在图中的补集，作为最大独立集(图的点子集中的点没有相连的）的近似是不合适的，如果对于那种每个分成两组，每组n/2个点，组内不连通，组外和任意一个点均相连的图，其最大独立集显然包含了所有点，用该算法得到的却是0个点，误差极大
## 图的着色问题的近似贪婪算法
检查某个点所有相邻点是否有从1（0代表无色）开始询问某个的color,如果没有则给它该color,否则color+1(color预先排好序）再检查其相邻点；依然是对于上述的图，如果顺序是轮流每组一个的话，这个算法会给出n种颜色，而实际两种颜色就够了
# 解非线性方程组的算法
对于连续的非线性方程的根的解法有如下数值分析方法
* 二分法
* 试位法
与二分法不同的是，该方法在计算下一个根不是用区间平分的结果而是使用区间端点对应函数值相连直线的截距
* 牛顿迭代法
原理是使用在x_n处切线的截距来使得x_n+1更加接近根；公式是x_n+1 = x_n-f(x_n)/f'(x_n),对于x^2-a=0，使用该公式会得到上一章牛顿法的结果；这个公式如果导数值远离0是成立的，否则会失败；而初始值和函数本身的一些特殊属性可能导致算法结果发散
