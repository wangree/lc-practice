## 输入增强和预构造
输入增强的典型例子是比较计数，分布计数，BM算法；预构造的典型例子是散列，B树
## 计数排序法
...
kmp算法是从左到右扫描模式的，主要查看当模式扫描到j时，当文本中字符为c且不匹配时，j-1个字母和c组成的字符串的后缀与模式的前缀重叠部分长度为模式的下一个状态(DFA[c][j])，即这时侯模式应该移动到前缀和刚才文本中j-1个字母（也即模式的后缀）及c组成的字符串对齐的位置
### KMP状态机的构造
...
### 线性探测散列表（闭散列，开式寻址）
元素所在位置（index)大于等于其散列值,于是我们用一个含有空元素数组的循环数组（查找到最后一个非空元素不匹配时，要接着遍历0位元素）就可以表示一个散列表，不需要多个链表，这时候的N/M就小于1了（M为数组大小）
