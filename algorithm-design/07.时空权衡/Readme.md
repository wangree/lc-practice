...
D_1关于模式和不匹配字符的研究 D_2关于自身重复性的研究 D_2的移动距离要大于等于hospool法的距离 这证明了haspool的正确性和bm的优点
## KMP算法
kmp算法是从左到右扫描模式的，主要查看当模式扫描到j时，当文本中字符为c且不匹配时，j-1个字母和c组成的字符串与模式的前缀重叠部分为模式需要移动的距离，或者说状态机的取值
## 散列法
需要做题深化认识
## 以B树作索引
